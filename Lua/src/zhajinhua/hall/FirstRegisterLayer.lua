...
function FirstRegisterLayer:createUI()
    ...
    --外框
    local window = display.newScale9Sprite("view/frame3.png", display.cx, display.cy, cc.size(600, 300))
               :addTo(self)
...
function FirstRegisterLayer:modifyNickname(newName)
    ...
    UserService:sendModifyUserInfoWithUnderWrite(userInfo:gender(), newName, userInfo.mobilePhoneNumber, userInfo:getUnderWrite(), true);
end
