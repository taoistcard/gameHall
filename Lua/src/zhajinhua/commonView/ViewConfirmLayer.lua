...
function ViewConfirmLayer:createUI()
    ...
    --外框
    local frame = display.newScale9Sprite("common/pop_bg.png", display.cx, display.cy, cc.size(690, 300))
               :addTo(self)
            :align(display.CENTER, display.cx, display.cy)

    --关闭按钮
    local exit = ccui.Button:create("common/close.png");
    exit:setPosition(cc.p(680,290));
    exit:addTo(frame);
    ...
    local content = display.newTTFLabel({text = self.desc,
                ...
                :align(display.LEFT_CENTER, 50, 220)
                :addTo(frame)
    ...
    --取消
    local button = ccui.Button:create("common/common_button2.png");
    button:setScale9Enabled(true);
    button:setContentSize(cc.size(180, 67));
    --button:setCapInsets(cc.rect(130, 33, 1, 1));
