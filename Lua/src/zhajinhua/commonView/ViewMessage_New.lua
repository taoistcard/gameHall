...
function ViewMessage:setChairArray(chairArr)
    self.chairArray = chairArr
end

function ViewMessage:setAllowChat(bflag)
...
function ViewMessage:createUI()
    ...
    --输入框区域
    local botBg = ccui.ImageView:create("view/bottom_bg.png");
    botBg:setScale9Enabled(true)
    botBg:setContentSize(cc.size(winWidth-8, 80));
    botBg:align(display.BOTTOM_CENTER, winWidth / 2, 3)
    self._bg:addChild(botBg)

    local inputbg = ccui.ImageView:create()
    inputbg:loadTexture("view/input_bg.png")
    inputbg:setScale9Enabled(true)
    inputbg:setContentSize(cc.size(420, 57))
    --inputbg:setCapInsets(cc.rect(90,30,1,1))
    inputbg:setPosition(cc.p(217, 40))
    botBg:addChild(inputbg)


    local chatEditBox = ccui.EditBox:create(cc.size(410, 55), "blank.png");
    ...
    chatEditBox:setAnchorPoint(cc.p(0,0.5));
    chatEditBox:setPosition(3, 25)
    ...
    --常用语
    local button = ccui.Button:create("view/btn_bg.png","view/btn_bg_selected.png","view/btn_bg_selected.png");
    ...
    button:align(display.CENTER, 465, 40)
    button:addTo(botBg)
    self.btnMsg = button

    local btnicon = ccui.ImageView:create("view/msg_icon.png");
    ...
    button:align(display.CENTER, 533, 40)
    button:addTo(botBg)
    self.btnFace = button

    local btnicon = ccui.ImageView:create("view/face_icon.png");
    btnicon:align(display.CENTER, 30, 30)
    btnicon:addTo(button)

    --发送按钮
    local button = ccui.Button:create("view/sendbtn_bg.png","view/sendbtn_bg_selected.png","view/sendbtn_bg.png");
    button:setPressedActionEnabled(true);
    ...
    button:align(display.CENTER, 613, 40)
    button:addTo(botBg)
    --self.btnMsg = button
...
function ViewMessage:showChatListLayer()
...
function ViewMessage:refreshNodeChat()
    
    if self.nodeChat == nil then
        local node = ccui.Layout:create()
        node:setContentSize(cc.size(645, 360))
        node:align(display.BOTTOM_LEFT, 12, 85)
    ...
    list:setDirection(ccui.ScrollViewDir.vertical)
    list:setBounceEnabled(true)
    list:setContentSize(cc.size(645, 340))
...
function ViewMessage:createQuickMsg()
    ...
    for k,v in ipairs(pathQuickMsg) do
        ...
        itemUnderWrite:setTextColor(cc.c4b(255,226,112,255))
...
function ViewMessage:msgFace(index)
    ...
    -- GameCenter:sendChatFaceMessage(index)
...
function ViewMessage:msgChat(message)
    ...
    -- GameCenter:sendChatTextMessage(255, message)
...
function ViewMessage:onUserMessage(event)
    if DateModel:getInstance():getListenChat() == false then
        return
    end    
    ...
    if self.chairArray ==nil then
        print("Error:ViewMessage:popMsg self.chairArray is nil")
        return
    end
...
function ViewMessage:getPostionIndex(chairId)
...
function ViewMessage:clearPlayerMsg()
