...
--是否同花
function GameLogic.checkSameColor(cards)
	local sameColor = false
	local kindArray = {}
	for i,v in ipairs(cards) do
		local kind = math.floor(v / 0x10 )
		table.insert(kindArray, kind)
	end
	local sameKindCount = 0
	for i=1,#kindArray do
		if kindArray[i] == kindArray[1] then
			sameKindCount = sameKindCount + 1
		end
	end
	if sameKindCount == #kindArray then
		sameColor = true
	end
	return sameColor
end
--是否顺子A,8,9
function GameLogic.checkShunZi(cards)
	local shunzi = {1,8,9}
	local sameCount = 0
	local isSz = false
...
function GameLogic.checkCardType(cards)
	local cardsClone = {}
	for i,v in ipairs(cards) do
		cardsClone[i] = v
	end
	local tonghua = GameLogic.checkSameColor(cardsClone)
	local Ashunzi = GameLogic.checkShunZi(cardsClone)
	local cardType = GameLogic.analysebCardData(cardsClone)
	local cardsNum = {}
	for i,v in ipairs(cardsClone) do
		cardsNum[i] = v%0x10
	end
	table.sort(cardsNum)
	local length = #cardsNum
	local sameNumCount = 0
	local flag = {}
	for i=1,length do
		flag[i]=0
	end
	for i=1,length do
		...
		if cardsNum[i] == cardsNum[j] then
			sameNumCount = sameNumCount + 1
		end

	end
	if sameNumCount == 0 then
		cardType = CARDS_TYPE.CT_1
		if cardsNum[1]==0x01 and cardsNum[3] == 0x0D then
			if cardsNum[3]-cardsNum[2]==1 then
				cardType = CARDS_TYPE.CT_1S
				if tonghua then
					print("同花顺")
					return CARDS_TYPE.CT_7
				end
			end
		else
			if cardsNum[3]-cardsNum[1]==2 then
				cardType = CARDS_TYPE.CT_1S
				if tonghua then
					print("同花顺")
					return CARDS_TYPE.CT_7
	...
	elseif sameNumCount == 1 then
		cardType = CARDS_TYPE.CT_2
	elseif sameNumCount == 2 then
		cardType = CARDS_TYPE.CT_3
	end
	-- print("sameNumCount",sameNumCount)
	...
	if Ashunzi then
		if tonghua then
			...
			print("顺子")
			return CARDS_TYPE.CT_1S
		end
	end
	if cardType == CARDS_TYPE.CT_3 then
		print("豹子")
		return CARDS_TYPE.CT_3
	end
	if tonghua then
		if cardType == CARDS_TYPE.CT_1S then
			print("同花顺")
			return CARDS_TYPE.CT_7
		else
			print("同花")
			return CARDS_TYPE.CT_6
		end		
	end
	return cardType
end
...
function GameLogic.analysebCardData(cards)
	...
	elseif #cardAnlyse.cardInfo[1] >= 1 then
		...
		if #cardAnlyse.cardInfo[2] == 0 and #cardAnlyse.cardInfo[3] == 0 and #cardAnlyse.cardInfo[4] == 0 then
			...
			elseif n >= 3 and GameLogic.isLine(cardAnlyse.cardInfo[1]) then--顺子
				cardType = CARDS_TYPE.CT_1S
			end
		end
	end
