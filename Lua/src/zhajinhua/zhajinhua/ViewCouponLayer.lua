...
function ViewCouponLayer:createUI()
	...
	if true then
	    ...
	    for i=1,3 do
	    	...
	    	--头像
	    	local headView = require("commonView.HeadView").new(i);
			...
			----数量
			local coupon = display.newSprite("common/ty_lq_icon.png"):addTo(contentBG):align(display.CENTER, pos.x+70, pos.y-25)
...
function ViewCouponLayer:queryHisCouponInfo()
    ...
    local wSubID = CMD_GameServer.SUB_GR_QUERY_NEARLY_DRAWED_USER

    local data = protocol.hall.treasureInfo_pb.CMD_GP_C_QueryVoucherCount_Pro()
    data.dwVoucherPoolType = 1
...
function ViewCouponLayer:onServerMessage(event)

    if event.subId == CMD_GameServer.SUB_GR_VOUCHER_BETTING_REPLY then--礼券投注
        print("礼券投注返回")
        local msg = protocol.hall.treasureInfo_pb.CMD_GP_VoucherBetting_Pro()
        msg:ParseFromString(event.data)
        self:processBetCallBack(msg)

    elseif event.subId == CMD_GameServer.SUB_GR_VOUCHER_DRAWING_REPLY then----礼券开奖
    	print("礼券开奖返回")
        local msg = protocol.hall.treasureInfo_pb.CMD_GP_VoucherDrawing_Pro()
        msg:ParseFromString(event.data)
        self:processOpenVoucherCallBack(msg)

    elseif event.subId == CMD_GameServer.SUB_GR_QUERY_VOUCHER_COUNT then----礼券查询总量
		print("礼券查询返回")
        local msg = protocol.hall.treasureInfo_pb.CMD_GP_QueryVoucherCount_Pro()
        msg:ParseFromString(event.data)
        self:processQureyVoucherCallBack(msg)

    elseif event.subId == CMD_GameServer.SUB_GR_QUERY_NEARLY_DRAWED_USER then
		print("历史奖励查询返回")
        local msg = protocol.hall.treasureInfo_pb.CMD_GR_NearlyDrawedUser_Pro()
        msg:ParseFromString(event.data)
...
--[[{
	...
	if msg and msg.lResultCode == 0 then
		...
		local userInfo = PlayerInfo:getMyUserInfo()
		userInfo:setCoupon(userInfo:getCoupon()+msg.dwAwardItemCount)
...
function ViewCouponLayer:processQureyVoucherCallBack(msg)
	...
	if self.couponCount and msg and msg.lVoucherInPoolCount then
...
function ViewCouponLayer:onClickBet()
    ...
    local wSubID = MSG_ID.SUB_C_USER_BET
    local curGameID = RunTimeData:getCurGameID() or 6
    print("curGameID",curGameID)
    

    local data 
    if curGameID == 200 then
    	data = protocol.game.doudizhu.doudizhu_pb.CMD_C_VoucherBetting_Pro()
    else
    	data = protocol.game.errendoudizhu.errendoudizhu_pb.CMD_C_VoucherBetting_Pro()
    end
    data.cbIsBet = 1
...
function ViewCouponLayer:qureyTotalCoupon()
	print("qureyTotalCoupon")

    local wMainID = CMD_GameServer.MDM_GR_USER
    local wSubID = CMD_GameServer.SUB_GR_QUERY_VOUCHER_COUNT
