...
function PreferentialRechargeLayer:createUI()
    ...
    local nickName = ccui.Text:create(FormotGameNickName(AccountInfo.nickName,6),"",24)
    ...
    local goldIcon = ccui.ImageView:create("show/hall_coin_icon.png")
    goldIcon:setPosition(11,17)
    goldIcon:setScale(0.9)
    goldBg:addChild(goldIcon)

    local gold = ccui.Text:create(FormatNumToString(AccountInfo.score),"",24)
    ...
    local bank = ccui.Text:create(FormatNumToString(AccountInfo.insure),"",24)
...
function PreferentialRechargeLayer:itemList()
	...
	for i=1,row do
		...
		for j=1,col do
	        ...
	        local priceLabel = ccui.Text:create(priceName[index],"",25)
...
function PreferentialRechargeLayer:buyHandler(sender)
    ...
    if PayInfo:getChargeStatusById(chargePackage.productType[index]) then
        ...
        local chargeLayer = require("show_zjh.popView_Hall.ChooseChargeLayer").new({index=index})
