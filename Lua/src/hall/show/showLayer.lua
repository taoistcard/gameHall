...
function showLayer:ctor()
    ...
    local backgroundSprite = cc.Sprite:create("show/show_background.jpg");
...
function showLayer:downloadAndEnterGame(gameName)
    ...
    local isFinished = cc.UserDefault:getInstance():getBoolForKey(curGameName.."_Finish", false)

    if self.isGameLoading == false and not isFinished then

        if gameName == "zhajinhua" then
            ...
            if iswifiConnect == 1 then
                self:startLoad(gamePackage.zhajinhua);
            else
                device.showAlert(
                    "提示",
                    "推荐使用WiFi下载，\n检测到您当前WiFi还未打开或连接，\n是否继续下载？",
    ...
    local unzipIsFinished = cc.UserDefault:getInstance():getBoolForKey(gamePackage.zhajinhua.file, false)
...
function showLayer:onFileDownloadFinish(errorCode)
    ...
    if errorCode ~= 0 then
        ...
        self.gameBtns[4]:setColor(cc.c3b(255,255,255))
        ...
        print("游戏已经下载成功")
        cc.UserDefault:getInstance():setBoolForKey(curGameName.."_Finish", true)
...
function showLayer:downloadProcessBar(i)
    ...
    progress_bg:setPosition(cc.p(self.posArr[i].x,self.posArr[i].y-100));
    ...
    progress:setPosition(cc.p(self.posArr[i].x-89,self.posArr[i].y-99));
    ...
    progressTxt:setPosition(cc.p(self.posArr[i].x,self.posArr[i].y-100))
...
function showLayer:showList()
    ...
    local isZJHFinished = cc.UserDefault:getInstance():getBoolForKey("zhajinhua_Finish", false)
    ...
    for i=1,4 do
        ...
        if i == 1 or i == 3 then
            ...
            lockSprite:setPosition(cc.p(self.posArr[i].x-80,self.posArr[i].y+50))
            self:addChild(lockSprite,10)
        end

        if OnlineConfig_review == "off" then
            if i == 4 and not isZJHFinished then
                armature:setColor(cc.c3b(150,150,150))
                self.lockSprite4 = display.newSprite("show/game_download.png")
                self.lockSprite4:setPosition(cc.p(self.posArr[i].x-80,self.posArr[i].y+50))
...
function showLayer:initBottomBtns()
    ...
    if OnlineConfig_review == "off" then
        ...
        local sprite = display.newSprite("show/btn_icon_dh.png");
        sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
        btn:addChild(sprite);

        local sprite = display.newSprite("show/btn_word_dh.png");
    ...
    if OnlineConfig_review == "off" then
        ...
        -- local sprite = display.newSprite("show/btn_icon_hd.png");
        -- sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
        -- btn:addChild(sprite);

        -- local sprite = display.newSprite("show/btn_word_hd.png");
        -- sprite:setPosition(btn:getContentSize().width/2-6, -2)
        -- btn:addChild(sprite);
    ...
    local sprite = display.newSprite("show/btn_icon_cz.png");
    sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
    btn:addChild(sprite);

    local sprite = display.newSprite("show/btn_word_cz.png");
    sprite:setPosition(btn:getContentSize().width/2-6, -2)
    btn:addChild(sprite);
    ...
    if OnlineConfig_review == "off" then
        ...
        local sprite = display.newSprite("show/btn_icon_xx.png");
        sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
        btn:addChild(sprite);

        local sprite = display.newSprite("show/btn_word_xx.png");
        sprite:setPosition(btn:getContentSize().width/2-6, -2)
        btn:addChild(sprite);
    ...
    local sprite = display.newSprite("show/btn_icon_sz.png");
    sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
    btn:addChild(sprite);

    local sprite = display.newSprite("show/btn_word_sz.png");
    sprite:setPosition(btn:getContentSize().width/2-6, -2)
    btn:addChild(sprite);
end

function showLayer:shopHandler()
    local shop = require("show.popView_Hall.ShopLayer").new(1)
    self:addChild(shop,1000)
end

function showLayer:noticeHandler()
    local setting = require("show.popView_Hall.NoticeLayer").new()
    self:addChild(setting,1000)
end

function showLayer:settingHandler()
    local setting = require("show.popView_Hall.SettingLayer").new()
    self:addChild(setting,1000)
end

function showLayer:onClickExchange()
    local newLayer = require("show.popView_Hall.ExchangeCouponLayer").new()
...
function showLayer:initTopBtns()
    ...
    local sprite = display.newSprite("show/btn_icon_sc.png");
    sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
    btn:addChild(sprite);

    local sprite = display.newSprite("show/btn_word_sc.png");
    sprite:setPosition(btn:getContentSize().width/2-6, -2)
    btn:addChild(sprite);

    startXPos = startXPos - 130
    ...
    local sprite = display.newSprite("show/btn_icon_mr_gift.png");
    sprite:setPosition(btn:getContentSize().width/2, btn:getContentSize().height/2)
    btn:addChild(sprite);

    local sprite = display.newSprite("show/btn_word_xs_gift.png");
    sprite:setPosition(btn:getContentSize().width/2-6, -2)
    btn:addChild(sprite);
    ...
    --每日金币
    local btn = ccui.Button:create("show/btn_mr_coin.png");
    btn:setPosition(cc.p(startXPos, yPos));
    btn:onClick(
        function()
    ...
    local sprite = display.newSprite("show/btn_word_mr_coin.png");
    sprite:setPosition(btn:getContentSize().width/2-6, -2)
    btn:addChild(sprite);
    ...
    if index == self.curIndex then
        ...
        elseif index == 4 then
            ...
            if OnlineConfig_review == "off" then
                ...
                if not isZJHFinished or not unzipIsFinished then --下载失败
        ...
        elseif index == 4 then
            ...
            if OnlineConfig_review == "off" then
                ...
                if unzipIsFinished then
                    --更新
                    require("updateSingleGame"):checkUpdate(self,1032,"zhajinhua")
                else
                    self:downloadAndEnterGame("zhajinhua")
                end
            else
...
function showLayer:getHallAnimation()
    ...
    if manager:getAnimationData("dating_game") == nil then
        manager:addArmatureFileInfo("hall_effect/game_icon/dating_game0.png","hall_effect/game_icon/dating_game0.plist","hall_effect/game_icon/dating_game.ExportJson")
    end
    local armature = ccs.Armature:create("dating_game")
    return armature;
end
...
function showLayer:getSelectedAnimation()
    ...
    if manager:getAnimationData("dating_xuanzhong") == nil then
        manager:addArmatureFileInfo("hall_effect/selected/dating_xuanzhong0.png","hall_effect/selected/dating_xuanzhong0.plist","hall_effect/selected/dating_xuanzhong.ExportJson")
    end
