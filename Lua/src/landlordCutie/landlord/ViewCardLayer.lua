...
function ViewCardLayer:ctor(dataModel)
    ...
    self.posTable[self.preID] = cc.p(-360,50)
...
function ViewCardLayer:showAllCardsDelay(msg, delay)
    --隐藏警报器
    self:hideJingBao(self.myChairID)
    self:hideJingBao(self.preID)
    self:hideJingBao(self.nextID)
...
function ViewCardLayer:sendCard()
    ...
    -- 发上家牌
    orignalPos = cc.p(display.cx + self.posTable[self.preID].x - 30, display.cy + self.posTable[self.preID].y - 10)
...
function ViewCardLayer:drawNextOutCards(cards, isRecover)
    ...
    --显示警报
    if self.nextCardsNum <= 2 then
        self:showJingBao(self.nextID, self.nextCardsNum)
    end
end

-- 出牌
function ViewCardLayer:drawPreOutCards(cards, isRecover)
    -- 清空
    self.outCardsPre:removeAllChildren()
    ...
    --设置地主角标
    if self.data.banker == self.preID and total>0 then
        local bankerlog_out = cc.Sprite:create("poker/banner_dizhu.png")        
        bankerlog_out:setPosition(self.bankerlog_outPos)
        bankerlog_out:setScale(0.55*rightCard:getScale())
        rightCard:addChild(bankerlog_out,1,101)
...
function ViewCardLayer:eventTiShi()
    ...
    if self.lastOutCards then
        ...
        --     local preCards = {}
        --     for k,v in ipairs(self.tishiCards) do
        --         table.insert(preCards, myCards[v])
        --     end

        --     local cardtype = GameLogic.analysebCardData(preCards)
        --     local a,b = GameLogic.autoCards(myCards, preCards)
        --     if a == true then
        --         self:cancelSelectCards()

        --         self.tishiCards = b
        --         for key, var in ipairs(b) do
        --             self.selCards[var] = 1
        --             local x,y = self.myCards[var]:getPosition();
        --             self.myCards[var]:setPosition(cc.p(x, y +20))
        --         end
        ...
        -- if self.tishiCards == nil then

        --     local cardtype = GameLogic.analysebCardData(self.lastOutCards)
        --     local a,b = GameLogic.autoCards(myCards, self.lastOutCards)
...
function ViewCardLayer:msgPassCard()
    ...
    DoudizhuInfo:sendPassCards()

end

function ViewCardLayer:msgOutCards(cards)
    print("ViewCardLayer:msgOutCards", self.myChairID)
...
function ViewCardLayer:showDiPai(msg)
    ...
    --显示底牌
    self:drawDiPai()
    -- print(self.myChairID, self.nextID, self.preID, #msg.cbBankerCard,msg.wBankerUser,msg.wCurrentUser,msg.cbBankerScore)
...
function ViewCardLayer:updateCardPos(cards)
    ...
    if self.data.banker == self.myChairID and rightCard and rightCard:getChildByTag(100) == nil then
        local bankerlog = cc.Sprite:create("poker/banner_dizhu.png")
...
function ViewCardLayer:recover(msg)
    ...
    print(#msg.cbBankerCard)
    dump(msg.cbBankerCard, "游戏底牌")
    ...
    for i=1, total, 1 do
        print("recovr---cards", i,  self.data.cards[i])
        local kind = math.floor(self.data.cards[i] / 0x10 );--math.random(0,3);
        local num = self.data.cards[i] % 0x10;--math.random(1,13);
          ...
          sequence = transition.sequence(
                            {
                                cc.DelayTime:create(interval),
                                cc.CallFunc:create(function() card:show(); end),
                                cc.MoveTo:create(interval, destPos)
                            }
                            )
        else
            sequence = transition.sequence(
    ...
    --设置地主角标
    if self.data.banker == self.myChairID then
        local bankerlog = cc.Sprite:create("poker/banner_dizhu.png")
        self.myCards[#self.myCards]:addChild(bankerlog,0,100)
        self.bankerlog = bankerlog
        bankerlog:setPosition(self.bankerlogPos)
...
--[[
    ...
    dump(msg.wTurnWiner, "wTurnWiner")
    dump(msg.cbTurnCardCount, "cbTurnCardCount")
    dump(msg.cbTurnCardData, "cbTurnCardData")
    if msg.wTurnWiner then
        ...
        elseif msg.wTurnWiner == self.preID then
            self:drawPreOutCards(msg.cbTurnCardData, true)
        end
    end
end

function ViewCardLayer:playEffectCT()
    -- local sprite = display.newSprite("play3/chuntian.png"):addTo(self.effectLayer):align(display.CENTER, display.cx, display.cy + 60)
    -- sprite:scale(10)
    ...
    -- --粒子效果
    -- local particle = cc.ParticleSystemQuad:create("particle/chuntian.plist")
...
function ViewCardLayer:playEffectShakeScreen()
    cc.Shaky3D:create(10, cc.size(15,10), 4, false)
