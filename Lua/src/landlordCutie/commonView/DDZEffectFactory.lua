...
function EffectFactory:getAnimationTask()
	local name = "doudizhu_renwu"

	local filePath = "effect/renwu/"..name..".ExportJson"
	local imagePath = "effect/renwu/"..name.."0.png"
	local plistPath = "effect/renwu/"..name.."0.plist"
...
function EffectFactory:getShopAnimation()
	local manager = ccs.ArmatureDataManager:getInstance()
	manager:removeArmatureFileInfo("effect/shangcheng_texiao.ExportJson")
	manager:addArmatureFileInfo("effect/shangcheng_texiao.ExportJson")
	local armature = ccs.Armature:create("shangcheng_texiao")
	armature:getAnimation():playWithIndex(0)
	return armature;
end
...
function EffectFactory:shakeScreen(time)
    ...
    local action = 
    cc.Sequence:create(cc.Repeat:create(
				    	cc.Sequence:create(
						    cc.DelayTime:create(0.005),
						    cc.CallFunc:create(function()
								runningScene:setPosition(x+math.random(-v,v), y+math.random(-v,v))
						    	end)), 
				    	time/0.02),
        				cc.CallFunc:create(function()
...
-- 	local filePath = "effect/zhu_bo_mm/"..name..".ExportJson"
--     local imagePath = "effect/zhu_bo_mm/"..name.."0.png"
--     local plistPath = "effect/zhu_bo_mm/"..name.."0.plist"
...
--主播MM动画
function EffectFactory:getZhuBoMMAnimation()
	local name = "zhubo_mm"
	local filePath = "effect/zhu_bo_mm/"..name..".ExportJson"
    local imagePath = "effect/zhu_bo_mm/"..name.."0.png"
    local plistPath = "effect/zhu_bo_mm/"..name.."0.plist"
...
--邀请有礼获得奖励动画
function EffectFactory:getInviteFriendAnimation()
	local name = "yyl_liquan"
	local filePath = "effect/recommand_award/"..name..".ExportJson"
    local imagePath = "effect/recommand_award/"..name.."0.png"
    local plistPath = "effect/recommand_award/"..name.."0.plist"
...
function EffectFactory:getMyselfPoChanArmature()
	local name = "ddz_pochan"

	local filePath = "effect/pochan/myself/"..name..".ExportJson"
	local imagePath = "effect/pochan/myself/"..name.."0.png"
	local plistPath = "effect/pochan/myself/"..name.."0.plist"
...
function EffectFactory:getOthersPoChanEffect()
	local animation = cc.Animation:create()
	for i=1,5 do
		animation:addSpriteFrameWithFile("effect/pochan/other/" .. i ..".png")
		-- display.newSpriteFrame("effect/pochan/other/" .. i ..".png")
	end
