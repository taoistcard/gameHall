...
function ViewChuPaiLayer:start(index)
	...
	if index == self.myChairID then
		...
		self.timer:startTimer(self.playConfig.cbTimeHeadOutCard, function() print("出牌超时") end, true, 5)--cbTimeHeadOutCard
		...
		--定时闹钟
		self.timer:setPosition(posTimer[index])
		self.timer:startTimer(self.playConfig.cbTimeHeadOutCard, function() print("其他玩家出牌超时") end, true, 5)--cbTimeHeadOutCard
	end
end
