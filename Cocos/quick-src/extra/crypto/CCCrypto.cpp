...
{
    if(memcmp(path, "assets//", sizeof(char)*7)==0)
    {
      path += 7;

      CCLOG("Crypto::MD5File %s", path);

      ssize_t size;
      unsigned char* buf = FileUtils::getInstance()->getFileData(path, "rb", &size);

      MD5_CTX ctx;
      MD5_Init(&ctx);
      MD5_Update(&ctx, buf, (int)size);
      MD5_Final(output, &ctx);
