...
@interface SVBaseWebViewController () <UIWebViewDelegate>
...
@implementation SVBaseWebViewController
...
- (void)viewWillAppear:(BOOL)animated {
    NSAssert(self.navigationController, @"SVBaseWebViewController needs to be contained in a UINavigationController. If you are presenting SVBaseWebViewController modally, use SVBaseModalWebViewController instead.");
...
{
    NSMutableURLRequest *mutableURLRequest = (NSMutableURLRequest*)request;
    NSURL* url = mutableURLRequest.URL;
    NSString* path = [url absoluteString];
    if([path hasPrefix:@"lccmd://"])
    {
        if([path hasPrefix:@"lccmd://GO_PAYMENT/"])//收到该广播
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CLOSE_SVModalWebViewController" object:nil userInfo:nil];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"GO_PAYMENT" object:nil userInfo:nil];
        }
        return NO;
    }
    return YES;
